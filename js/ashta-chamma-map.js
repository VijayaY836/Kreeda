  ...
  window.initAshtaChammaWorldMap = function (container) {
    ...
    function updatePinVisuals() {
      Array.from(pinsGroup.children).forEach(gPin => {
        ...
        const pinScale = (isHovered ? 1.25 : 1) / Math.sqrt(zoom);
        gPin.setAttribute('transform', `translate(${loc.x}, ${loc.y}) scale(${pinScale})`);
        const filterG = gPin.querySelector('g[filter]');
        if (filterG) {
          filterG.setAttribute('filter', isHovered ? 'url(#active-pin-glow)' : 'url(#antique-pin-shadow)');
        }
      });
    }
    ...
    function renderTooltip() {
      ...
      tooltipContainer.innerHTML = `
        <div style="width: 270px; background-color: #FAF4E7; border: 1.5px solid #5B3922; border-radius: 8px; padding: 14px; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.45); position: relative; text-align: left; background-image: radial-gradient(ellipse at top left, #fffdf8, #f5ebd3); pointer-events: none;">
          ...
          <div class="ac-tooltip-notch" style="position: absolute; width: 10px; height: 10px; background-color: #FAF4E7; transform: rotate(45deg); pointer-events: none;"></div>
        </div>
      `;
      ...
      const notchEl = tooltipContainer.querySelector('.ac-tooltip-notch');
      if (notchEl) {
        const notchLeftPx = Math.max(16, Math.min(cardWidth - 16, pinX - left));
        const notchStyle = isBelow
          ? '-top: 6px; border-top: 1.5px solid #5B3922; border-left: 1.5px solid #5B3922;'
          : '-bottom: 6px; border-bottom: 1.5px solid #5B3922; border-right: 1.5px solid #5B3922;';
        notchEl.style.cssText = `position: absolute; width: 10px; height: 10px; background-color: #FAF4E7; transform: rotate(45deg); ${notchStyle} left: ${notchLeftPx}px; margin-left: -5px; pointer-events: none;`;
      }
    }
