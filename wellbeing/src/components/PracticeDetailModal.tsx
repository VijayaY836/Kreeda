...
export const PracticeDetailModal: React.FC<PracticeDetailModalProps> = ({ practice, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#2B1B12]/60 flex items-center justify-center p-4" onClick={onClose}>
      <div
        className={`bg-[#FAF4E5] border-[3px] border-[#5C140F] w-full max-h-[88vh] relative wb-fade-in ${
      ...
      >
        {hasGif ? (
          <>
            ...
            <div className="md:w-1/2 bg-[#E4D19E] border-t-[3px] md:border-t-0 md:border-l-[3px] border-[#5C140F] flex items-center justify-center p-3 shrink-0">
          ...
          <>
            {practice.image && (
              <div className="relative h-44 w-full border-b-[3px] border-[#5C140F] overflow-hidden">
        ...
        <button
          onClick={onClose}
          className={`absolute top-4 right-4 p-1.5 border-2 border-[#5C140F] text-[#5C140F] cursor-pointer ${practice.image || hasGif ? 'bg-[#FAF4E5]' : 'bg-[#EFDFB8]'}`}
        >
          <X className="w-4 h-4" />
        </button>
      </div>
