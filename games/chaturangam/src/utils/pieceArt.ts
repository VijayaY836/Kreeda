/* ============================================================
   ...
   other piece on the board. Attribution: Colin M.L. Burnett (Cburnett).
   Chaturangam pieces are original hand-drawn SVGs for this project —
   the Raja wears a jewelled turban, deliberately with no cross.
   ============================================================ */
...
/* viewBox for the shared/Chaturangam set (40x44); chess pieces use their native 45x45 */
export const ART_VIEWBOX: Record<Variant, string> = {
  chaturanga: '0 0 40 44',
  chess: '0 0 45 45',
};

const ART_BASE: Partial<Record<PieceLetter, string>> = {
  P: `<path d="M20 6c3 0 5 2.2 5 5 0 1.9-1 3.4-2.4 4.3 2.6 1.3 4.2 3.7 4.6 7.2H12.8c.4-3.5 2-5.9 4.6-7.2A5 5 0 0 1 15 11c0-2.8 2-5 5-5z" ${SS}/><path d="M14 24h12l2 8H12z" ${SS}/><path d="M10 32h20l2 6H8z" ${SS}/>`,
  N: `<path d="M25 6c-6 0-9.5 3-12 7.5-1.6 2.8-3.5 4-5 4.6-1.2.5-1.3 1.6-.2 2l4.2 1.5-1.4 3.2c-.5 1.2.4 2.2 1.6 1.8l3-1c1.6 3 1.4 5.6.6 8.4h13c1.4-6.6 1.6-12.4-.2-17.4C27.2 11.5 26.6 8.6 25 6z" ${SS}/><circle cx="22.6" cy="14.2" r="1.5" fill="var(--pc-stroke)"/><path d="M11 32h18l2 6H9z" ${SS}/>`,
  R: `<circle cx="20" cy="17" r="10" ${SS}/><circle cx="20" cy="17" r="3" fill="var(--pc-stroke)"/><path d="M20 7v20M10 17h20M13 10l14 14M27 10L13 24" stroke="var(--pc-stroke)" stroke-width="1.8" stroke-linecap="round"/><path d="M11 32h18l2 6H9z" ${SS}/>`,
  E: `<ellipse cx="10.4" cy="17.5" rx="5.2" ry="6.8" ${SS}/><ellipse cx="29.6" cy="17.5" rx="5.2" ry="6.8" ${SS}/><path d="M20 6.2c4.7 0 7.8 3.2 7.8 7.7 0 4.3-2.2 7.5-3.4 9.6h-8.8c-1.2-2.1-3.4-5.3-3.4-9.6 0-4.5 3.1-7.7 7.8-7.7z" ${SS}/><path d="M15.4 3.6h9.2v3.4h-9.2z" ${SS}/><path d="M17.2 1.6h1.9v2h-1.9zM20.9 1.6h1.9v2h-1.9z" ${SS}/><path d="M20 20.5c0 4.2.3 6.7 2.6 8.4 1.5 1.1 1 3-.9 3" fill="none" stroke="var(--pc-stroke)" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"/><path d="M16 22.6l-1.7 3.5M24 22.6l1.7 3.5" fill="none" stroke="var(--pc-stroke)" stroke-width="2" stroke-linecap="round"/><circle cx="16.6" cy="14.6" r="1.3" fill="var(--pc-stroke)"/><circle cx="23.4" cy="14.6" r="1.3" fill="var(--pc-stroke)"/><path d="M11 32h18l2 6H9z" ${SS}/>`,
  M: `<path d="M20 4.4l2 4.2 3.8-1-1.5 4.1C26.9 13.2 28.4 15.6 28.4 18.4c0 3.8-3.8 6.3-8.4 6.3s-8.4-2.5-8.4-6.3c0-2.8 1.5-5.2 4.1-6.7L14.2 7.6l3.8 1z" ${SS}/><circle cx="20" cy="17.4" r="2.3" fill="var(--pc-stroke)"/><path d="M13.3 26.7h13.4l1.5 5H11.8z" ${SS}/><path d="M10 32h20l2 6H8z" ${SS}/>`,
};

const ART_VARIANT: Record<Variant, Partial<Record<PieceLetter, string>>> = {
  chaturanga: {
...
export interface PieceInfo {
  n: string; t: string; en: string; tag: string; worth: string;
  how: string; note: string;
}

export const PIECE_INFO: Record<Variant, Partial<Record<PieceLetter, PieceInfo>>> = {
  chaturanga: {
...
export const ORDER: Record<Variant, PieceLetter[]> = {
  chaturanga: ['P', 'N', 'E', 'R', 'M', 'K'],
  chess: ['P', 'N', 'B', 'R', 'Q', 'K'],
};

export const VARIANT_INFO: Record<Variant, { title: string; native: string; lead: string; sides: { w: string; b: string } }> = {
  chaturanga: {
    title: 'Chaturangam',
    native: 'చతురంగం · चतुरङ्ग · the ancestor of chess',
  ...
  chess: {
    title: 'Chess',
    native: 'the descendant · Shatranj → Europe → the modern game',
...
export const PIECE_WORTH: Partial<Record<PieceLetter, number>> = { P: 1, N: 3, B: 3, R: 5, Q: 9, E: 1.5, M: 2, K: 0 };
